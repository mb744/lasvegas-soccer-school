...
### Enabling ACS (email + SMS outreach)

Email is fully declarative — flip a single GitHub variable, push, the deploy provisions the ACS resource, an Email Communication Service, and an Azure-managed `*.azurecomm.net` sender domain. Sender address is `donotreply@<auto-generated>.azurecomm.net`. SMS phone numbers must be purchased separately because Azure doesn't expose phone-number purchasing through Bicep.

#### Email — one repo variable, then push

```powershell
gh variable set ENABLE_ACS --body "true" --repo <owner>/<repo>
# Push or workflow_dispatch the deploy. Bicep creates the ACS + Email Service +
# managed domain, wires the connection string and from-address into Container
...
1. Portal → your Communication Service → **Phone numbers** → **Get** → **Toll-free** (cheapest, ~$2/month). Choose your area code, finish purchase. Note the E.164 number.
2. Toll-free numbers in the US require **TFN registration** before they can send marketing/transactional SMS. In the same Phone numbers blade → click your number → **Toll-free verification** → submit. Carrier review takes 1-2 weeks.
3. Once verified, set the repo variable:
   ```powershell
   gh variable set ACS_SMS_FROM_NUMBER --body "+18005551212" --repo <owner>/<repo>
   ```
4. Push or workflow_dispatch the deploy. The container app picks up `Acs__SmsFromNumber` and SMS outreach starts working.
